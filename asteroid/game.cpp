...
void Game :: handleInput(const Interface & ui) {
   ...
   if(ui.isSpace()) {
      if(!gunType) {
         Bullet tempBul;
         tempBul.fire(ship, ship.aimAngle(), tempBul.getVelocity().getSpeed());
         bullets.push_back(tempBul);
      } else {
         for(int i = 0; i < 15; i++) {
            Bullet tempBul;
            tempBul.fire(ship, (i*24) + ship.aimAngle(), tempBul.getVelocity().getSpeed());
            bullets.push_back(tempBul);
         }
      }
   }
...
void Game :: createAsteroids() {
   ...
   for (int i = 0; i < ((level*2)+10); i++) {
      ...
      newRock->fire(newRock->getPoint(), random(0, 360), newRock->getVelocity().getSpeed());
...
void Game :: createAsteroid(int type, Rock rock) {
   ...
   if(type == 1) {
      ...
      newRock1->addY(1);
      newRock1->fire(newRock1->getPoint(), random(0, 360), rock.getVelocity().getSpeed());
      ...
      newRock2->addY(-1);
      newRock2->fire(newRock2->getPoint(), random(0, 360), rock.getVelocity().getSpeed());
      ...
      newRock3->addX(2);
      newRock3->fire(newRock3->getPoint(), random(0, 360), rock.getVelocity().getSpeed());
